...
(function () {
    ...
    minesPercent.addEventListener("change", function () {
        GenerateField();
    });
    GenerateField();
})();
function getButtonAround(a, b) {
    return document.getElementById(`btn_${a}_${b}`);
}
    ...
    if (!btnClicked.disabled) {
        ...
        if (9 == v) {
            ...
            if (0 == btnClicked.value) {
                let btn = getButtonAround(a - 1, b - 1);if (btn) {
                    CheckBoom(btn);
                }
                btn = getButtonAround(a - 1, b);if (btn) {
                    CheckBoom(btn);
                }
                btn = getButtonAround(a - 1, b + 1);if (btn) {
                    CheckBoom(btn);
                }
                btn = getButtonAround(a, b - 1);if (btn) {
                    CheckBoom(btn);
                }
                btn = getButtonAround(a, b + 1);if (btn) {
                    CheckBoom(btn);
